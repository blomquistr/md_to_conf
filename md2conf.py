...
PARSER.add_argument(
    "--pat",
    help="Confluence Personal Access Token if $CONFLUENCE_PERSONAL_ACCESS_TOKEN not set.",
)
PARSER.add_argument(
    "-o",
    "--orgname",
...
try:
    ...
    if ORGNAME is not None:
        LOGGER.debug("ORGNAME: %s", ORGNAME)
        if ORGNAME.find(".") != -1:
            LOGGER.debug("ORGNAME provided as full URL, omitting formatting")
            CONFLUENCE_API_URL_TMP = "https://%s" % ORGNAME
        else:
            LOGGER.debug("Formatting CONFLUCE_API_URL with ORGNAME...")
            CONFLUENCE_API_URL_TMP = "https://%s.atlassian.net/wiki" % ORGNAME
            LOGGER.debug("CONFLUENCE_API_URL_TMP: %s", CONFLUENCE_API_URL_TMP)
    else:
...
def convert_iframe_macros(html):
    ...
    html_tag = (
        '<p><ac:structured-macro ac:name = "html" ><ac:plain-text-body > <![CDATA['
    )
...
def get_page(title):
    ...
    if PROPERTIES:
        url = "%s,%s" % (
            url,
            ",".join("metadata.properties.%s" % v for v in PROPERTIES.keys()),
        )
    ...
    if len(data["results"]) >= 1:
        ...
        link = "%s%s" % (CONFLUENCE_API_URL, data["results"][0]["_links"]["webui"])

        try:
            properties = data["results"][0]["metadata"]["properties"]
        except KeyError:
        ...
        page_info = collections.namedtuple(
            "PageInfo", ["id", "version", "link", "properties"]
        )
...
def add_local_refs(page_id, title, html):
    ...
    if not MARKDOWN_SOURCE in ref_prefixes or not MARKDOWN_SOURCE in ref_postfixes:
        LOGGER.warning(
            "Local references weren't processed because "
        ...
        if links:
            for link in links:
                ...
                if ref not in headers_map:
                    LOGGER.error(
                        "Invalid '%s' local link detected: '%s'. Please update the source file or change the markdown source (-mds) parameter.",
                        MARKDOWN_SOURCE,
                        ref,
                    )
                    sys.exit(1)
                ...
                if result_ref:
                    ...
                    if VERSION == 1:
                        ...
                        replacement = (
                            '<ac:link ac:anchor="%s"><ac:plain-text-link-body><![CDATA[%s]]></ac:plain-text-link-body></ac:link>'
...
def add_pages_refs(html):
    ...
    if not PAGES_MAP:
        LOGGER.warning(
            "Page to page references weren't processed because "
        ...
        for link in links:
            ...
            for key in PAGES_MAP:
                if ref.startswith(key):
                    path = os.path.join(
                        PAGES_MAP[key], urllib.parse.unquote(ref[len(key) :])
                    )
                    ...
                    try:
                        ...
                        LOGGER.error(
                            'Cannot find local file "%s" when resolving page to page link "%s"',
                            path,
                            ref,
                        )
...
def create_page(title, body, ancestors):
    ...
    new_page = {
        ...
        "ancestors": ancestors,
        "metadata": {"properties": {"editor": {"value": "v%d" % VERSION}}},
    }
    ...
    if response.status_code == 200:
        ...
        link = "%s%s" % (CONFLUENCE_API_URL, data["_links"]["webui"])
        ...
        if img_check or local_ref_check or properties or ATTACHMENTS or LABELS:
            LOGGER.info(
                "\tAttachments, local references, content properties or labels found, update procedure called."
            )
            update_page(
                page_id, title, body, version, ancestors, properties, ATTACHMENTS
            )
        else:
            if GO_TO_PAGE:
    ...
    if response.status_code == 200:
        ...
        if properties:
            ...
            for key in properties:
                ...
                prop_json = {
                    "key": key,
                    "version": {"number": properties[key]["version"]},
...
def main():
    ...
    with codecs.open(MARKDOWN_FILE, "r", "utf-8") as mdfile:
        ...
        html = markdown.markdown(
            html, extensions=["tables", "fenced_code", "footnotes"]
        )
    ...
    if page:
        ...
        if PROPERTIES:
            for key in PROPERTIES:
                if key in page.properties:
                    properties[key] = {
                        "key": key,
                        "version": page.properties[key]["version"]["number"] + 1,
        ...
        update_page(
            page.id, title, html, page.version, ancestors, properties, ATTACHMENTS
        )
    else:
